...
from h_tsp import LKHSolver, readDataFile, HTSP_PPO, utils, VecEnv, RLSolver, GreedySolver, FarthestInsertSolver
...
def parse_args():
    ...
    parser.add_argument("--batch_size", type=int, default=16, help="Evaluate batch size")
...
def main(args):
    ...
    vec_env = VecEnv(k=k, frag_len=frag_len, max_new_nodes=max_new_cities, max_improvement_step=0)
    results = np.array([])
    start_t = time.time()
    for i in range(0, data.shape[0], bsz):
        ...
        while not vec_env.done:
            ...
            s, r, d, info = vec_env.step(a, rl_solver, frag_buffer=model.val_frag_buffer)
