...
def tsplib2sample(problem):
    ...
    for n in range(1, len(pos_x_list)):
        sample['customers'].append({'position': (pos_x_list[n], pos_y_list[n]), 'demand': 1})
...
def lkh_solver(sample, atsp=False, runs=-1):
    ...
    if runs == -1:
        route = lkh.solve('/usr/local/bin/LKH', problem=problem, max_trials=problem.dimension, runs=1)#, max_trials=problem.dimension, runs=10)
    else:
        route = lkh.solve('/usr/local/bin/LKH', problem=problem, max_trials=problem.dimension, runs=runs)
    route = [r-1 for r in route[0]]
    customer_pos_list = []
    if 'max_pos' in sample:
        depot_pos = (sample['depot'][0]*(sample['max_pos'][0]-sample['min_pos'][0]) + sample['min_pos'][0],
                        sample['depot'][1]*(sample['max_pos'][1]-sample['min_pos'][1]) + sample['min_pos'][1])
        for cus in sample['customers']:
            cus_pos = (cus['position'][0]*(sample['max_pos'][0]-sample['min_pos'][0]) + sample['min_pos'][0],
                        cus['position'][1]*(sample['max_pos'][1]-sample['min_pos'][1]) + sample['min_pos'][1])
    ...
    if not atsp:
        ...
        for i in range(1, len(route)-2):
            dist += node_distance(customer_pos_list[route[i]-1], customer_pos_list[route[i+1]-1])
