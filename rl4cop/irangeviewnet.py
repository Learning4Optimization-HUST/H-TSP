...
def upsample(idims, odims, stride=(2, 2), mode='deconv'):
    if mode == 'deconv':
        return nn.Sequential(
            nn.ConvTranspose2d(idims, odims, stride, stride=stride,
                               bias=False),
    ...
    def __init__(self, idims, odims, stride=1):
        ...
        if idims == odims and stride == 1:
            self.conv2 = nn.Sequential(
                ...
                nn.Conv2d(6 * idims, odims, 1, stride=1, padding=0,
                          bias=False),
                nn.BatchNorm2d(odims, eps=BN_EPS, momentum=BN_MOMENTUM))
            self.shortcut = None
        else:
...
class iRangeViewNet(nn.Module):
  ...
  """
    def __init__(self, in_channels, init_type='kaiming_uniform'):
        ...
        self.res2 = iBottleneckBlock(idims=in_channels,
                                     odims=2 * in_channels,
                                     stride=(2, 2))
        self.res3 = iBottleneckBlock(idims=2 * in_channels,
                                     odims=4 * in_channels,
                                     stride=(2, 2))
        self.deconv2 = upsample(2 * in_channels,
                                odims=in_channels,
                                stride=(2, 2))
        self.deconv3 = upsample(4 * in_channels,
                                odims=in_channels,
                                stride=(4, 4))
        self.conv_out = nn.Conv2d(3 * in_channels,
                                  in_channels,
                                  3,
                                  stride=1,
                                  padding=1)
